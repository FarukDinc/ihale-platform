...
**✅ 26-7 (=UV-4) KAPANDI — TOP-1000 (2 Ağu):** İdare ad-ortası boşluk ("ALTINPAR K"→ALTINPARK, "BET ON"→BETON, "O TEL"→OTEL). Çözüm `backend/idare_ad_temizle.py`: heuristik aday (dupe-grup + wrap imzası, meşru "E Tipi/1 Nolu/A.Ş." korunur) → **Gemini `gemini-3.1-flash-lite` doğrulama** (DeepSeek'ten iyi: MSB "BLG→BİLGİ" tuzağına düşmedi, OTEL'i düzeltti) → dry-run CSV → `--apply --min-guven 0.85`. **CANLI: 138 idare ilanlar+dt+takip_idareler'de düzeltildi (0 hata), MV+idare_tur tazelendi.** Tuzaklar (hepsi giderildi): (a) DeepSeek 40'lık öbekte max_tokens aşıp JSON kırpıyordu → öbek 20 + 8000 token; (b) takip_idareler service_role UPDATE grant'ı yoktu (403/42501) → `migration_qa_takip_idareler_grant.sql` + `--sadece-takip`; (c) origin 2r/s → apply throttle+429 retry. ⏭️ **OPSİYONEL uzun kuyruk:** kalan ~39K küçük idare için tam-katalog paslı tur (`--limit 0`); değer düşük, acele değil, önce dry-run resumable yapılmalı (mevcut dry-run CSV'yi yalnız sonda yazıyor → uzun run'da çökerse kayıp).
