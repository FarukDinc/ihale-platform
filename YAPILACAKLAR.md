...
- [~] 🟢 **SONUÇ/FİRMA VERİSİ — PIPELINE ÇALIŞIYOR, GERÇEK VERİ AKIYOR (8 Tem, kullanıcı onayıyla):** Firma/yüklenici tarafı kuruldu ve **gerçek veri çekildi.** Yapılanlar:
  ...
  - **KALAN 3 ADIM:** (a) 🔲 **cron'a ekle** (onay bekliyor — güvenlik katmanı blokladı): `run_scraper.sh`'e `$VENV/python ekap_sonuc_backfill.py --reset --max-pages 50` satırı (her gece en yeni 5000 sonuçlanmışı tarar, biriktirmek için). (b) 🔲 **`firma-analiz.html` "Sonuçlar" sekmesini** `ihale_sonuclari` GROUP BY kazanan_firma ile doldur (toplam iş/ciro/ort. tenzilat/kazanma) — placeholder hâlâ "Yakında". (c) 🔲 **Derin tarihsel backfill (Faz 5):** 1.68M'i taramak için proxy (IP ban) + Supabase ölçek gerek — ayrı proje. (d) `yukleniciler` tablosu boş kaldı (Design A yolu doldurmuyor); istenirse ihale_sonuclari'dan agregatla doldurulur.
