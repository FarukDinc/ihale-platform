...
def dt_detay_getir(client_ekap, dt_ihale_token, dt_idare_token):
    ...
    try:
        r = client_ekap.get(ARAMA_ENDPOINT, params={
...
def main():
    ...
    with httpx.Client(timeout=60) as client, \
         httpx.Client(verify=ssl_ctx(), timeout=30.0, follow_redirects=True) as client_ekap:
        ...
        if args.dry_run:
            ...
            if not batch:
                ...
                return
            for row in batch:
                veri = dt_detay_getir(client_ekap, row["dt_ihale_token"], row["dt_idare_token"])
