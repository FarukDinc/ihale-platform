...
| ✅ B4+B2b | `migration_idare_dizin_detsis_fulljoin.sql` | isim-anahtar + tek-evren | DT İSİMLE join → varyant sessiz-0; ihale-çıpalı LEFT → yalnız-DT kurum yok | ✅ dt_idare_ozet_mv'ye detsis_no + idare_dizin_json birleşik-k (COALESCE(detsis,'n:'||idare)) ön-toplama + FULL OUTER. DT 2,09M→3,01M (isimle kayıp %30 geri), 143 DT-only kurum görünür, BEL-PA 1 satır, ihale/harcama regresyon YOK (5 Ağu) | ~1g |
