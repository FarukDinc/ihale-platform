...
| 9 | Devam eden/tamamlanan ayrımı, TL+EUR, iş-grubu | 🟡 | ~~Düşük~~ **Orta** | 🟨 **KISMEN** — sözleşme durumunun kazımasız gelen parçası TESLİM EDİLDİ: **fesih (2.205) + tasfiye/devir (828)** `tum_teklifler`'den türetildi, firma karnesinde koşullu risk şeridi + ihale detayında uyarı, gece tazelemesi cron'da. Kalan (devam eden/tamamlanan) hâlâ ⛔ **VERİ ENGELİ (23 Tem ölçüldü)** — arayüz işi DEĞİL. `ihale_sonuclari`'nda `is_baslama_tarihi` / `is_bitis_tarihi` / `is_suresi_gun` **539.203 satırın hepsinde NULL**, `ham_json` da %100 boş → ayrım türetilemez. Önce scraper'ın sonuç detayından bu alanları çekmesi gerek. Boş/uydurma sütun EKLENMEDİ. TL+EUR: sözleşmelerde para birimi kolonu yok (hepsi TL) — rakiptekinin karşılığı bizde yok |
